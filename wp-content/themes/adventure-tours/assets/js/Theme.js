...
Theme.tourBookingForm = {
	...
	initFixedTourBookingButtonScroller:function(buttonBoxSelector, invisibleBookingFormClass){
		if (!buttonBoxSelector || !invisibleBookingFormClass) return;
		var fixedBtnBox = jQuery(buttonBoxSelector),
			tourBookingForm = jQuery(this.formSelector);
		if (fixedBtnBox.length < 1 || tourBookingForm.length < 1) return;

		var recheckTout = null,
			self = this,
			oldState = null,
			checkBookingVisibility = function(){
				var newState = self.isElementInViewport(tourBookingForm);
