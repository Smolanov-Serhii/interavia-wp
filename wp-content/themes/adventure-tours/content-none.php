...
<?php elseif ( is_search() ) : ?>
	<div class="page-404__box padding-top padding-bottom">
		...
		<?php } ?>
			<input type="search" class="search-field" placeholder="<?php esc_attr_e( 'Напишите Ваш запрос', 'adventure-tours' ); ?>" value="<?php echo get_search_query(); ?>" name="s">
...
<?php else : ?>
	<div class="page-404__container stick-to-top stick-to-bottom">
		<div class="page-404__content">
			...
			<p class="page-404__description"><?php esc_html_e( 'ОЙ, страница не найдена' ); ?></p>
			<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="search-form page-404__form page-404__form--style1">
			<?php if ( adventure_tours_check( 'is_wpml_in_use' ) ) { ?>
				<input type="hidden" name="lang" value="<?php echo esc_attr( ICL_LANGUAGE_CODE ); ?>">
			<?php } ?>
				<input type="search" class="search-field" placeholder="<?php esc_attr_e( 'Напишите запрос...', 'adventure-tours' ); ?>" value="<?php echo get_search_query(); ?>" name="s">
				<i class="fa fa-search"></i>
				<input type="submit" class="search-submit" value="<?php esc_attr_e( 'Искать', 'adventure-tours' ); ?>">
			</form>
		</div>
	</div>
<?php endif; ?>
